...
namespace Database
{
    ...
    sql::PreparedStatement* DBWorker::prepareStatement(const std::string& sql)
    {
        checkConnection();
        if (m_Connection) {
            try {
                ...
                LOG_ERROR("DBWorker {} prepare statement failed: {}", m_ID, e.what());
            }
        }
        return nullptr;
    }

    void DBWorker::insert(sql::PreparedStatement* pstmt)
    {
        if (!pstmt) return;

        checkConnection();
        if (!m_Connection) {
            LOG_ERROR("DBWorker {} no valid connection for insert", m_ID);
            return;
        }
        ...
        try {
            ...
            LOG_ERROR("DBWorker {} insert failed: {}", m_ID, e.what());
            // 发生异常时尝试重连后重试
            checkConnection();
            if (m_Connection) {
                try {
                    pstmt->execute();
                    LOG_INFO("DBWorker {} insert retried successfully", m_ID);
                } catch (const sql::SQLException& e2) {
                    LOG_ERROR("DBWorker {} insert retry failed: {}", m_ID, e2.what());
                }
            }
        }
    }

    void DBWorker::inquire(sql::PreparedStatement* pstmt, std::function<void(sql::ResultSet*)>&& callback)
    {
        if (!pstmt) 
            return;
        ...
        checkConnection();
        if (!m_Connection) {
            LOG_ERROR("DBWorker {} no valid connection for inquire", m_ID);
            return;
        }
        ...
        try {
            ...
            LOG_ERROR("DBWorker {} inquire failed: {}", m_ID, e.what());
            // 发生异常时尝试重连后重试
            checkConnection();
            if (m_Connection) {
                try {
                    std::unique_ptr<sql::ResultSet> result{ pstmt->executeQuery() };
                    callback(result.get());
                    LOG_INFO("DBWorker {} inquire retried successfully", m_ID);
                } catch (const sql::SQLException& e2) {
                    LOG_ERROR("DBWorker {} inquire retry failed: {}", m_ID, e2.what());
                }
            }
        }
    }
